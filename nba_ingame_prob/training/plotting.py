...
def plot_calibration_curve(
    ...
    """
    Create a calibration plot showing predicted vs actual probabilities.
    ...
    ax1.plot(mean_predicted_value, fraction_of_positives, "s-", label=model_name, linewidth=2, markersize=8)
    ...
    ax2.hist(y_prob, bins=n_bins, alpha=0.7, density=True, edgecolor='black')
    ax2.set_xlabel("Predicted Probability")
    ax2.set_ylabel("Density")
    ax2.set_title(f"Distribution of Predictions - {model_name}")
    ax2.grid(True, alpha=0.3)
...
def save_calibration_plot(
    ...
    fig = plot_calibration_curve(y_true, y_prob, model_name)
    plot_path = output_dir / f"{prefix}_{model_name.lower().replace(' ', '_')}.png"
