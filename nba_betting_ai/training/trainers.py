...
from nba_betting_ai.training.metrics import compute_all_metrics
from nba_betting_ai.training.mlflow_utils import (
    log_artifact_path,
    log_metrics_dict,
    log_params_flat,
    ensure_experiment,
...
def _log_artifacts_to_mlflow(model_obj: Any, config: OmegaConf, scalers_path: Path, 
                           model_suffix: str = '.pkl', artifact_folder: str = 'artifacts') -> None:
    """
...
def _log_csv_artifact(data: pd.Series, filepath: Path, save_local: bool) -> None:
    """
    Log a pandas Series as CSV artifact, either locally or to MLflow directly.
    
    Args:
        data: Pandas Series to save
        filepath: Local file path (only used if save_local=True)
        save_local: Whether to save locally or use temporary file
    """
    if save_local:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as tmp:
            ...
            log_artifact_path(Path(tmp.name))
            os.unlink(tmp.name)
...
def train_bayesian(config: OmegaConf, data: DataSet, scalers: dict, extra_tags: dict | None = None, pruner: Callable[[int, dict[str, float]], bool] | None = None, save_local_artifacts: bool = True) -> TrainingResult:
    ...
    if seed is not None:
        try:
            ...
            pass
    model = __import__('nba_betting_ai.model.bayesian', fromlist=['BayesianResultPredictor']).BayesianResultPredictor(**model_init).to(device)
    ...
    with _start_training_run(run_name, config['experiment_name']) as run:
        ...
        try:
            if not scalers_src.exists():
                with open(scalers_src, 'wb') as f:
                    pickle.dump(scalers, f)
            _log_artifacts_to_mlflow(best_src, config, scalers_src, '.pth')
        except Exception as e:
            print(f"Warning: Failed to log artifacts to MLflow: {e}")
